...
| Workout portal    | `·`                         | **`x`** если есть completed session (как completion); duration — в сессии / Истории |
...
| Workout duration в сетке    | **Не в сетке.** Completed session → глиф `x` (presence). `durationSeconds` — в W1-C / Истории / H6b |
...
Portal columns: presence `x` · checklist % · navigate 06/07
