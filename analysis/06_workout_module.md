...
Не Strong / не Apple Health: сессия дня, подходы, шаблоны, «прошлый раз», секундомер → длительность на сессии; в сетке Habits — `…` / `x`.
...
**Edit duration:**
- **C:** sheet H6b → только `durationSeconds`.
- **B:** tap таймер → тот же sheet (prefill = elapsed) → СОХРАНИТЬ = `finishSessionWithDuration` (завершить с введённым `M:SS`, не wall-clock). Валидация как у ЗАКОНЧИТЬ; иначе toast.
...
| Forgot finish (hours later) | Tap таймер → задать `M:SS` → завершить; или ЗАКОНЧИТЬ → при необходимости edit на C |
...
| С | Как |
|---|-----|
| Habits grid | `…` in_progress · `x` completed; жесты входа из 05 |
