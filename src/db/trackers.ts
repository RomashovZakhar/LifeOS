...
/** Rewrite sortOrder from ordered ids (left→right on grid). */
export async function reorderTrackers(orderedIds: string[]): Promise<void> {
  await db.transaction("rw", db.trackers, async () => {
    for (let i = 0; i < orderedIds.length; i++) {
      const id = orderedIds[i]!;
      const t = await db.trackers.get(id);
      if (!t || t.sortOrder === i) continue;
      await db.trackers.put({ ...t, sortOrder: i });
    }
  });
}
